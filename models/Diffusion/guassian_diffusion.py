...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        def process_xstart(x):
            ...
            if clip_denoised:
                _min = x.min()
                _max = x.max()
                normalized_img = (x - _min)/ (_max - _min)
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        res = mask.clone().detach()
        res_t = self.q_sample(res, t, noise=noise) #add noise to the segmentation channel (eq 3)
        #x_t = res_t.clone().detach()     
        x_t=x_start.float()
        x_t[:, -1:, ...]=res_t.float()
        terms = {}
